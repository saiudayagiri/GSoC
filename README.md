...
As for lessons learned, everything has been a lesson. Starting as a newcomer, nothing was easy. I didn’t even know that the code we write needs to run through a CI pipeline. When I made my first PR for my proposal, every step was a learning opportunity, from reading the codebases and fundamentals during the community bonding period to creating the environment. The initial days were very slow, as everything was new—I wasn’t very familiar with Git, nor with SymPy and its modules. Every line I read required a break, as there were many other libraries and functions involved. It often felt like looping between functions and methods, reading about what each does, their limitations, and what alternative methods might achieve what we needed that the current function could not. It was a lot of time-consuming work, and every week I used to spend two nights sitting to get it finished—it would become morning. It’s about everything: if you get a determinate system, you have to go through and understand what it is; indeterminate, inconsistent, and under-constrained systems were all fresh concepts and required a bit of research before diving in. For finding inconsistencies, as the issue involved inconsistent systems, it was like going back to fundamentals about matrices, ranks, augmented matrices, RREF, etc., for PR [#28113](https://github.com/sympy/sympy/pull/28113). And for the issue, there were a lot of refinements and enhancements, as the solve_for_reaction_loads used to return an index key error for many situations like no reaction symbols passed, fewer reaction symbols passed, inconsistent systems, or simple beams with a roller or pin—all used to return the same error. The work involved adding a lot of validation checks for the arguments, checking all boundary conditions, and everything. As more and more time passed, and after reviewers' suggestions and the RREF suggestion as the best approach, it came to an end. The point_cflexure issue was again head-scratching, as it becomes very easy to find roots, but not knowing how to find sign changes for symbolic roots was a problem. Along with that, finding a constant zero region and skipping it to avoid failing with a "not implemented" error like before, and combining both, was the challenge. For this, I made sure to add as many test cases as possible. This is where I understood why my mentor emphasized TDD (Test-Driven Development) while making my draft in the community bonding period. This is where things started becoming better and better, as I had a huge test case setup. But the big issue was that I didn’t know we could test our local changes with the updated code. By this time, all I was doing was plotting bending moments, finding roots for those expressions manually, adding them in test cases, updating the point_cflexure method in the module, and testing using pytest sympy, which pulled me back so much. This was a very wrong way, which I didn’t know. As my project started approaching midterm slowly, I understood about venv; again, it took a full two days to set it up, and I got lost somewhere doing all these and needed to close all environments, uninstall SymPy, Python, VSCode, and set up everything again. Along with this, trying to get the logic for point_cflexure—it’s funny to reveal, but the eureka moment for point_cflexure was while I was bathing. No surprise at all, saying I was thinking about this while I was bathing, because all the initial days were like that—the work used to come into my dreams, so no wonder while bathing. Then suddenly, the two-pointer approach struck from my DSA prep, having solved around 100s of problems using pointers and not applying them is a shame. I tried imagining using a two-pointer approach, as we are mainly dealing with roots; it is guaranteed that no point in between any two roots crosses the x-axis, which is a great point, as any point in between the current root and left root has the same bending region, either positive or negative, same to the right. Checking these regions left and right, and if they are opposite, it is clear to say that it’s a contraflexure. This is when the solution came, which even works for symbolic stuff. As I worked more and more, the module became clearer, and everything became easier—Git, modules, methods, SymPy, and my efficiency showed growth. Along with that, I could find other issues too, as point_cflexure was failing with constant zero regions; I could find the same type of code used for max_bending and max_shear, and thought I had a feeling that this also fails for zero regions, and then the proof came. So, the more you understand the codebase, the more you get it, and it becomes easier. Also, while solving all these, I felt very bad for always passing reaction symbols to the solve method and always assigning symbols to apply_support. Lucas, who built the Column module, made it very simple by tracking all the symbols internally. That is when I realized it is okay to break the existing rules to make the module better. That is when I followed a similar approach and updated the whole module to make the solve method user-friendly by tracking reaction symbols internally from apply_support. But this breaks .rst file people are using this module and familiar with the pattern; breaking this might be problematic. My mentor said to keep it user-friendly and also try not to break existing functionality. Then I refined it again using both arguments and internal symbols, adding logic to avoid duplicates passing to linsolve. I just made sure we combine user symbols and internal symbols, make a unique list out of it, and pass them, and raise an error when duplicates exist in the users' symbols, as linsolve fails it. Every issue taught me something and added progress and made me smarter. There are times with no results for a while and a high the next day after finding it. And after this, this PR came out [#28263](https://github.com/sympy/sympy/pull/28263), which made my life easier to get this also done [#28305](https://github.com/sympy/sympy/pull/28305). The more you stick to a problem, the higher chance you get of solving it. The wider the test cases are, the better the code turns out. The more you understand the codebase and the basics, the easier it becomes to work on it. The more you put time into debugging, the more you have a chance to figure it out. All these lessons came from each of the issues above, but Structure2D taught all these lessons together. Every method was hard to understand at the beginning; every step was hard—lots of trying, failing, trying, failing—not knowing where the flaw is, where the issue is, and where I am lacking. As this module involved many methods—unwrapping, apply_load, unwrapping structure, add_member, and everything—this really tested me. After all this, it made me start from scratch again: new module, new theory, large thesis of Alex based on which the module is developed, very limited knowledge of debugging. It nearly took a month to get clarity on understanding the module and all the methods. The clarity for half of the work came from the thesis, as the thesis gave a very clear idea of how each projected load should go into beam and column and how each member angle is compensated for unwrapped structures. As my knowledge in this domain is very limited, I didn’t even have any problems to try with, not many test cases. Again, my mentor helped and gave a few problems, resources, and one or two problems started giving correct results. All the way, one good thing is none of my changes ever broke existing test cases, which gave me confidence that I am moving in the right direction. But a few problems were giving very contradictory solutions: the thesis gives reactions in terms of E, I, A when they are symbols and gives numeric when they are numeric, but my approach always gives numeric, which is close to the real answers but never depends on E, I, A. One thing was clear: there is something wrong with the implementation, and we are using beam and column modules which are purely developed for 1D structures, and trying to use them didn’t work for me. I went back to the thesis step by step; I started debugging each and everything from load expressions, support expressions, angles, signs, and everything. One thing that was missing in the module is the manual Alex algorithm converts these global displacements to local displacements—this was missing. And the manual thesis code adds constants for all shear, bending, deflection, slope, axial, and extension, but the beam and column are handling them differently: the beam doesn’t add the constants, the column does, and here we have additional coupling equations, and this is not working by solving reactions alone using them. If we try to use beam’s shear, as it didn’t have a constant term while applying boundary conditions, the shear term becomes zero and limit(0-) runs into an infinite loop. So I added that extra algorithm and rewrote the entire solve_reaction_loads method using the manual Alex thesis, and that’s when everything was good and giving nice results. The only case still missing was the hinge condition, but the module had no method for hinge application, so I added that and updated apply_load to support that, and that’s where the beauty came, and all started working. I added a huge variety of test cases—not a wide variety of structures, but a wide variety of loads and member angles—used Alex algorithm to test manually and Structure2D to test the module, and both gave the same good results. That’s where I felt I did something and was happy with Structure2D, and later parts became easy as adding plots and other refinements were good.
- Now good with Git
- Good with venv
- Good with terminal
- Handling large codebases better
- Debugging
- Open-source

Overall, a better programmer now.
